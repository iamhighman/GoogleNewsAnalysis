...
require_once './lib/ClassParser.php';
header('Content-Type: text/plain; charset=utf-8');

// get latest Medialens alert 
// (change this URL to whatever you'd like to test)
//$url = 'http://www.cna.com.tw/News/aSOC/201309190339-1.aspx';
//$filename = $_GET[id];
//$html = file_get_contents("./html/".$filename.".html");

$parser = new Parser(); 

$url = $_GET[url];
$html = $parser->fetchWeb($url);
...
if ($result) {
    ...
    $table = array_count_values(explode(" ", strip_tags($content)));
    asort($table);    
    if(count($table)>20) print_r($table);
} else {
