...
load.data.in.range <- function(start.date="2013-10-01",end.date="2013-10-13",use.cache=T) {
  if (use.cache) {
    ...
    ofilename = sprintf('./gnews/output/P1/data.rda')
    ...
    ofilename = sprintf('./gnews/output/P1/tdmat.rda')
    save.RData(td.mat,ofilename)    
  }
...
run.nmf <- function(mat,ks=2:6,k=0) {
  ...
  if (1) {
    ...
    print(proc.time() - ptm)
    ofilename = sprintf('./gnews/output/P1/nmf_res_k%d.rda',k)
...
explore.nmf <- function(k=0,use.cache=T,plot.heatmap=T,plot.wordcloud=T) {
  ...
  if (use.cache) {
    ifilename = sprintf('./gnews/output/P1/tdmat.rda')
    td.mat = load.RData(ifilename) 
    words = rownames(td.mat)

    ifilename = sprintf('./gnews/output/P1/nmf_res_k%d.rda',k)
...
doc2country.matrix <- function(use.cache=T) {
  if (use.cache) {
    ifilename = sprintf('./gnews/output/P1/data.rda')
    text = load.RData(ifilename) 
  }
...
mat = load.data.in.range(start.date="2013-10-01",end.date="2013-10-05",use.cache=F)
