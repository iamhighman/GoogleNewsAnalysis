...
load.data <- function(sqlstr = NULL) {
  ...
  if (is.null(sqlstr)) {
    ...
    sqlstr = 'select * from O_300_1 limit 20;'    
  }
...
corpus2matrix <- function(corpus,ngram=0,fname='1',use.dict=T,lo.bound=0.01,hi.bound=0.5) {
  ...
  if (use.dict) {
    ifilename = sprintf('./gnews/output/olympic/dict.rda')
  ...
  print(td.mat)
  if (!is.null(fname)) {
    ofilename = sprintf('./gnews/output/olympic/tdmat-%s-ngram%d.rda',fname,ngram)
...
load.data.in.range <- function(start.date="2013-10-01",end.date="2013-10-14",use.cache=T) {
  if (use.cache) {
    ifilename = sprintf('./gnews/output/olympic/data.rda')
    text = load.RData(ifilename) 
    ifilename = sprintf('./gnews/output/olympic/tdmat.rda')
    td.mat = load.RData(ifilename) 
  }
  else {
    text = load.data(sqlstr = sprintf('select ID, COUNTRY, ENG, TIMESTAMP from O_300_1 where datediff(TIMESTAMP,"%s")>=0 and datediff(TIMESTAMP,"%s")<0',start.date,end.date))    
    ofilename = sprintf('./gnews/output/olympic/data.rda')
    ...
    ofilename = sprintf('./gnews/output/olympic/tdmat.rda')
    save.RData(td.mat,ofilename)    
  }
...
load.data.in.range.outlet <- function(start.date="2013-10-01",end.date="2013-10-14",use.cache=T) {
  if (use.cache) {
    ifilename = sprintf('./gnews/output/olympic/outlet.rda')
    text = load.RData(ifilename) 
  }
  else {
    start.date="2013-10-01" 
    end.date="2013-10-14"
    text = load.data(sqlstr = sprintf('SELECT source, country, count(id) FROM `O_300_1` where datediff(TIMESTAMP,"%s")>=0 and datediff(TIMESTAMP,"%s")<0 group by SOURCE',start.date,end.date))    
    ofilename = sprintf('./gnews/output/olympic/outlet.rda')
    save.RData(text,ofilename)      
  }
}
...
explore.nmf <- function(k=0,use.cache=T,plot.heatmap=T,plot.wordcloud=T) {
  ...
  if (use.cache) {
    ...
    ifilename = sprintf('./gnews/output/olympic/nmf_res_k%d.rda',k)
...
doc2country.matrix <- function(use.cache=T) { #revised into time period verison
  if (use.cache) {
    ifilename = sprintf('./gnews/output/olympic/data.rda')
    text = load.RData(ifilename) 
    ifilename_p1 = sprintf('./gnews/output/P1/data.rda')
...
doc2cluster.matrix <- function(k=6,use.cache=T,plot.heatmap=F) {
  ...
  if (use.cache) {
    ...
    ifilename = sprintf('./gnews/output/olympic/nmf_res_k%d.rda',k)
    res = load.RData(ifilename)   
    ifilename_t = sprintf('./gnews/output/olympic/data.rda')
...
#  p3 <- p3 + geom_line(aes(linetype=V2), size = 1.5) + geom_point(aes(shape=V2), size = 4) + scale_shape(solid=FALSE)
  p3 <- p3 + geom_line(size = 3)
  p3 <- p3 + labs(colour ="Theme",shape="themes", x = "News Date",y = "News Frequency",title = "News Frequency Curve of Themes")
  p3 <- p3 + theme(plot.title = element_text(size = rel(1)))
  p3 <- p3 + theme(text = element_text(size=48), axis.title.x = element_text(size=48), axis.title.y = element_text(size=48), axis.text.x = element_text(angle = 90, hjust = 1, size=24), axis.text.y = element_text(size=24)) 
  p3
}
...
mat = load.data.in.range(start.date="2014-02-01",end.date="2014-03-01",use.cache=F)
for (ki in 7:10) run.nmf( as.matrix(mat) ,k=ki)
