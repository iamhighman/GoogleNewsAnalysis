...
#  p3 <- p3 + geom_line(aes(linetype=V2), size = 1.5) + geom_point(aes(shape=V2), size = 4) + scale_shape(solid=FALSE)
  ...
  p3 <- p3 + theme(plot.title = element_text(size = 14))
  p3 <- p3 + theme(text = element_text(size=12), axis.title.x = element_text(size=12), axis.title.y = element_text(size=12), axis.text.x = element_text(angle = 90, hjust = 1, size=12), axis.text.y = element_text(size=12)) 
  p3
}
...
doc2cluster.matrix.theme.bar <- function(k=6,use.cache=T,th=1) {
  ...
  p4 <- p4 + geom_bar(stat = "identity") + geom_point() + geom_text(aes(label=as.numeric(V2)),size=6,colour="red")
  p4 <- p4 + labs(x = "Countries",y = "News volume",title = paste("News Frequency Bar of Theme", th))
  p4 <- p4 + theme(plot.title = element_text(size = 14))
  p4 <- p4 + theme(text = element_text(size=12), axis.title.x = element_text(size=12), axis.title.y = element_text(size=12)) 
  p4 <- p4 + theme(axis.text.x = element_text(size=12, angle = 90, hjust = 1), axis.text.y = element_text(size=12))
  p4
}
...
if (0) {
 ...
 for (ki in 6:6) run.nmf( as.matrix(mat) ,k=ki)
